...
def segmented(lines: gpd.GeoSeries) -> gpd.GeoSeries:
    def _segmented_linestring(linestring: LineString) -> list[LineString]:
        return [
    ...
    def _segments_from_geom(geom) -> list[LineString]:
        if geom is None:
            return []
        ...
        if isinstance(geom, LineString):
            return _segmented_linestring(geom)
        if isinstance(geom, MultiLineString):
            segments: list[LineString] = []
            for part in geom.geoms:
                if not part.is_empty:
                    segments.extend(_segmented_linestring(part))
            return segments

        # Skip unsupported geometries (e.g., GeometryCollection).
        return []

    return gpd.GeoSeries([segment for geom in lines for segment in _segments_from_geom(geom)])
